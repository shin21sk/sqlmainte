...
|------|------|
| OS | Windows 10 / 11 |
| ランタイム | 不要（.NET Framework 4.8 は Windows 10 / 11 に標準搭載） |
...
実行ファイルは `bin/Release/net48/SqlMainte.exe` に生成されます。
...
| Microsoft.Extensions.Configuration.Json | 10.0.3 | appsettings.json 読込 |
| Microsoft.Extensions.Configuration.Binder | 10.0.3 | 設定オブジェクトへのバインド |
| System.Text.Json | （トランジティブ） | JSON シリアライズ（バイナリ列変換） |
